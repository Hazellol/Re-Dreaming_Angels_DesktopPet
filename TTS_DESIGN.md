...
- 用户在 `参考音频/` 里补充音频后，只需按同样格式追加到 `.list` → 重新生成 `emotions.json` 即可扩充
...
| **③ 精简推理服务** | 自研 `server.py`（FastAPI，只依赖推理核心）+ 依赖清单 + 启动脚本 | 需在用户的 GPT-SoVITS 环境调试 |
...
5. **阶段 ① 是否现在就做**（本项目可以立刻生成千夏的 `emotions.json` + 生成工具，不依赖任何环境）。
...
**若你的版本报错**（如 v2ProPlus 参数不兼容）：把报错原文发给本项目本项目来适配；也可在「高级(JSON)」里直接改：
```jsonc
...
- 分句流水线、试听页、爱芮/南宫语音包
- 待用户提供 GPT-SoVITS 环境信息后：**针对 v2ProPlus 的参数适配**
